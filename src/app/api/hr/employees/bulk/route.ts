...
import { requireAuth, apiError, createAuditLog } from "@hrms/lib/api-utils";
import { canExportEmployees, canBulkImportEmployees } from "@hrms/lib/permissions";
import {
  buildExcelBuffer,
  getRowValue,
  getRowDateValue,
  ...
  parseExcelBuffer,
  parseOptionalNumber,
} from "@hrms/lib/excel-utils";
import type { EmploymentType, Prisma, UserStatus } from "@prisma/hrms-client";
import { DEFAULT_PASSWORD } from "@/lib/auth-defaults";
import { deactivateWorknestUserByEmail, syncWorknestUserFromHrms } from "@/lib/people-sync";
...
function buildPartialUpdate(
  ...
  // Accept older HRMS salary column names when Incentive/Reimbursement are absent.
  if (!hasRowValue(row, "Incentive", "incentive")) {
    const special = parseOptionalRowNumber(
      row,
      "Special Allowance",
      "Special Allowance",
      "special_allowance",
    ...
    const hra = parseOptionalRowNumber(row, "HRA", "HRA", "hra");
    if (special.error) errors.push(special.error);
    else if (hra.error) errors.push(hra.error);
    else if (special.value !== undefined || hra.value !== undefined) {
      assign("incentive", "Incentive", (special.value ?? 0) + (hra.value ?? 0));
    }
  }
  if (
    !hasRowValue(row, "Reimbursement", "reimbursement") &&
    hasRowValue(row, "Internet Allowance", "internet_allowance", "internetAllowance")
  ) {
    const parsed = parseOptionalRowNumber(
      row,
      "Internet Allowance",
      "Internet Allowance",
      "internet_allowance",
    ...
    else if (parsed.value !== undefined) assign("reimbursement", "Reimbursement", parsed.value);
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    rawRows.forEach((row, index) => {
      ...
      const incentive =
        incentiveResult.value ?? specialAllowanceResult.value ?? hraResult.value ?? 0;
      const reimbursement =
        reimbursementResult.value ?? internetAllowanceResult.value ?? 0;
    ...
    for (const row of validRows) {
      if (row.isUpdate && row.userId) {
        ...
        if (updatedUser.status !== "ACTIVE") {
          await deactivateWorknestUserByEmail(updatedUser.email).catch((error) =>
