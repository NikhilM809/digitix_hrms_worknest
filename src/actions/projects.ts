...
function statusPatch(from: ProjectStatus, to: ProjectStatus, userId: string, completion?: Date | null) {
  const now = new Date();
...
export async function updateProject(projectId: string, formData: FormData) {
  ...
  if (isAdminLike(user.role)) {
    ...
    if (currencyId) {
      const currency = await prisma.currency.findFirst({ where: { id: currencyId, active: true } });
      if (!currency) return { error: "Select a valid active currency." };
