...
export async function generateInvoices(formData: FormData) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      const existingBatches = await tx.invoiceBatch.findMany({ select: { invoiceNumber: true } });
      ...
      for (const [index, project] of projects.entries()) {
        const already = await tx.invoice.findFirst({ where: { projectId: project.id } });
        if (already) {
...
async function uniqueInvoiceNumber(invoiceNumber: string) {
  const [batch, invoice] = await Promise.all([
    prisma.invoiceBatch.findUnique({ where: { invoiceNumber } }),
...
export async function uploadHistoricalInvoice(formData: FormData) {
  ...
  revalidatePath("/dashboard");
  revalidatePath("/reports");
  for (const project of projects) revalidatePath(`/projects/${project.id}`);
...
export async function markInvoicePaid(invoiceId: string) {
  ...
  if (invoice.batchId) {
    await prisma.$transaction([
      prisma.invoiceBatch.update({ where: { id: invoice.batchId }, data: { status: "PAID" } }),
      prisma.invoice.updateMany({ where: { batchId: invoice.batchId }, data: { status: "PAID" } }),
    ]);
  } else {
...
export async function markBatchPaid(batchId: string) {
  ...
  await prisma.$transaction([
    prisma.invoiceBatch.update({ where: { id: batchId }, data: { status: "PAID" } }),
    prisma.invoice.updateMany({ where: { batchId }, data: { status: "PAID" } }),
  ]);
  await notifyAdmins({
    title: "Bill marked as paid",
